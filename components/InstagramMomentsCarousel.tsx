...
export default function InstagramMomentsCarousel() {
  const [currentIndex, setCurrentIndex] = useState(0)
  const [likedMoments, setLikedMoments] = useState<Set<number>>(new Set())

  useEffect(() => {
    const duration = 32000 // 32 seconds total
    const itemDuration = duration / moments.length // 4 seconds per item
  ...
  const toggleLike = () => {
    setLikedMoments((prev) => {
      ...
      if (newLiked.has(currentIndex)) {
        newLiked.delete(currentIndex)
      } else {
        newLiked.add(currentIndex)
      }
      return newLiked
    })
  }

  const isLiked = likedMoments.has(currentIndex)

  const currentMoment = moments[currentIndex]

  return (
    <div className="instagram-moments-container">
      {/* Instagram Post Card */}
      <article className="instagram-post">
        {/* Header */}
        <header className="instagram__header">
          <div className="instagram__header-avatar">
            <Image
          ...
          <div className="instagram__header-info">
            <h4>Vishal Lakshmi Narayanan</h4>
            <p>Journey Through Tech</p>
          </div>
        </header>

        {/* Media Section with actual image */}
        ...
        <footer className="instagram__buttons">
          <div className="instagram__buttons-left">
            <svg
              className={`instagram__icon instagram__icon--heart ${isLiked ? "instagram__icon--heart-liked" : ""}`}
              fill="none"
          ...
          <div className="instagram__buttons-right">
            <svg
        ...
        <div className="instagram__footer">
          <div className="instagram__footer-date">
            <Calendar />
            <span>{currentMoment.date} • {currentIndex + 1}/{moments.length}</span>
          </div>
          <h3 className="instagram__footer-title">{currentMoment.title}</h3>
          <p className="instagram__footer-description">
